...
def generate_forcefield(template_string, parameters, FFtype = None, outfile = None):
    ...
    if FFtype is not None:
        ...
        elif (FFtype.strip().upper() == 'REAX2') or (FFtype.strip().upper() == 'REAXFF2'):
            reax_forcefield(filestring=template_string).write_formatted_forcefields(outfilename = outfile)
            template_string = None

    if outfile is not None and template_string is not None:
